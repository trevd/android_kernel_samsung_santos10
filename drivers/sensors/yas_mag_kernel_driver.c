		...
		struct device_attribute *attr, char *buf)
{
	...
	err4 =
		pcbtest.offset_control_measurement_and_set_offset_register(
			&x, &y1, &y2);
	/* dir should be returned as 0 as requested by QA team. */
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (input_data == NULL) {
		rt = -ENOMEM;
		YLOGE
			(("%s: Failed to alloc input_data device\n", __FUNC__));
		goto err;
	}
	...
	if (rt) {
		YLOGE(("%s: Unable to register input_data device: %s\n",
	...
	if (rt) {
		YLOGE(("%s: sysfs_create_group failed[%s]\n",
		__FUNC__, input_data->name));
		goto err;
	}
	sysfs_created = 1;
	...
	if (input_raw == NULL) {
		rt = -ENOMEM;
		YLOGE(("%s: Failed to allocate input_raw device\n" __FUNC__));
		goto err;
	}
	...
	if (rt) {
		YLOGE(("%s: Unable to register(raw) : %s\n"
