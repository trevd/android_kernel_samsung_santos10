...
static int get_q(struct uart_hsu_port *up, int *cmd, int *offset, int *value)
{
	...
	if (!CIRC_CNT(circ->head, circ->tail, MAXQ)) {
		//HSU_DEBUG("[HSU](%s)(HSU Port (%d)) GetQCnt(%d) \n", __func__, up->index, CIRC_CNT(circ->head, circ->tail, MAXQ) );
...
/* FIXME: AT_DISTRIBUTOR does not work cause of this */
#if 0
#ifdef CONFIG_SERIAL_MFD_HSU_CONSOLE
	if (index == CONFIG_SERIAL_MFD_HSU_CONSOLE_PORT)
		uport->use_dma = 0;
				...
				const struct pci_device_id *ent)
{
	...
	dev_info(&pdev->dev,
		"PCI ID: %04x:%04x FUNC: %d driver: %ld addr:%llx len:%llx\n",
...
static void qwork(struct work_struct *work)
{
	...
	if (up->suspended) {
		pm_runtime_put(up->dev);
		HSU_DEBUG("[HSU] (HSU PORT (%d))-(%s) suspended(%d) \n",
					up->index, __func__, up->suspended);
...
/*
  FIX ME :
  device can not enter suspend mode because of unbalanced ref count of
  hsu module runtime PM. To fix it, remove run-time feature from
  suspend function of hsu module. It's hack patch so we should revert
  it after appling hsu driver for kernel3.4
*/
