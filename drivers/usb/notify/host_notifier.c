...
struct host_notify_type type_define[] = {
	{
		.name = "default",
		.type = NOTIFY_DEFAULT,
		.enable_vbus_drive = 1,
	...
	{
		.name = "general_connector",
		.type = NOTIFY_GENERALHOST,
		.enable_vbus_drive = 1,
		.enable_noti = 1,
	},
	{
...
static int host_notifier_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(type_define); i++) {
		host_notify_type_register(&ninfo.pdata->ndev,
				&type_define[i]);
	}
	host_notify_set_type(&ninfo.pdata->ndev, NOTIFY_DEFAULT);
