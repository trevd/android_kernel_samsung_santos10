...
#ifdef CONFIG_THERMAL
static int dts_read_temp(struct thermal_zone_device *tzd,
			 long *temp)
{
	u32 eax, edx;
	struct temp_data *tdata = tzd->devdata;
...
static struct thermal_zone_device_ops dts_tzd_ops = {
	.get_temp = dts_read_temp,
};
				...
				unsigned int cpu, int pkg_flag)
{
...
#ifdef CONFIG_THERMAL
	sprintf(tzd_name, "CORE%d", tdata->cpu_core_id);

	tdata->tzd = thermal_zone_device_register(
		(char *)tzd_name, 0, 0, tdata, &dts_tzd_ops,
		0, 0, 0, 0);
	...
	if (IS_ERR(tdata->tzd))
		...
		dev_info(&pdev->dev,
			 "ThermalZone for DTS CORE%d is added",
			 tdata->cpu_core_id);
#endif
...
#ifdef CONFIG_THERMAL
	if (tdata->tzd)
		thermal_zone_device_unregister(tdata->tzd);
#endif
