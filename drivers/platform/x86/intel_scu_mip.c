...
/* KFT_CHECK: Added comment here for Satha to look into change that removed
 * the do/while loop that used to be around this call to
 * rpmsg_send_raw_command(), now just calls and takes ret while not checking
 * if its EIO to msleep() anymore
 */
#ifdef CONFIG_XEN
		if (xen_start_info) {
