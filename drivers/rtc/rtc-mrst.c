...
#ifdef CONFIG_XEN
	if (xen_start_info)
		xen_register_gsi(rtc_irq, -1, 0, 1);
#endif
...
static void __devexit vrtc_mrst_exit(void)
{
...
static struct rpmsg_driver vrtc_mrst_rpmsg_driver = {
...
{
	return register_rpmsg_driver(&vrtc_mrst_rpmsg_driver);
}

static void __exit vrtc_mrst_rpmsg_exit(void)
{
	return unregister_rpmsg_driver(&vrtc_mrst_rpmsg_driver);
}
