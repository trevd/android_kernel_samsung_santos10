...
static bool fw_updater(struct ts_data *ts, char *mode)
{
	...
	/* Read hw_id and version from firmware file */
	ts->fw_info->hw_id = (int)fw->data[0xb102];
	...
	if (!strcmp("force", mode)) {
		...
		if (ts_read_reg_data(ts->client, REG_FW_DATA, buf, 4) < 0) {
			pr_info("tsp: fw. ver. read failed.");
			goto out;
		}

		ts->fw_info_ic.hw_id = (int)buf[2];
						...
						const struct i2c_device_id *id)
{
	...
	ts->client->irq = gpio_to_irq(ts->platform_data->gpio_irq);
