...
#define DET_CHECK_TIME_MS	50		/* 50ms */
...
static void determine_jack_type(struct sec_jack_info *hi)
{
	...
	while (gpio_get_value(pdata->det_gpio) ^ npolarity) {
		...
		/* When reading adc is failed,
		 * it makes it possible to avoid watchdog error */
		if(adc < 0) {
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	ret = gpio_request(pdata->det_gpio, "DET_3.5");
	if (ret) {
	...
	ret = request_irq(hi->det_irq, sec_jack_detect_irq,
		...
		IRQF_ONESHOT | IRQF_NO_SUSPEND, "sec_headset_detect", hi);
	if (ret) {
