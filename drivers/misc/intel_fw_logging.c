...
#define output_str(ret, out, size, label, a...)				\
	do {								\
		if (out) {						\
...
static irqreturn_t fw_logging_irq_thread(int irq, void *ignored)
{
	...
	if (trace_hdr.cmd & TRACE_IS_ASCII) {
		...
		while (trace < end) {
			len = strnlen(trace, trace_hdr.size);
			if (!len) {
				trace++;
				continue;
			}
			...
			u32 *faberr_dwords, int max_dw_left)
{
	...
	if (num_err_log) {
		...
		while (i < num_err_log && i < max_dw_left) {
			...
			output_str(ret, buf, size, out, "error_addr: 0x%X\n",
				   *(faberr_dwords + i + 1));
			...
			output_str(ret, buf, size, out, "Agent Index:");
			...
			output_str(ret, buf, size, out, "Command: %d\n",
				   log.fields.cmd);

			output_str(ret, buf, size, out, "Code: %d\n",
				   log.fields.err_code);
		...
		output_str(ret, buf, size, out, "Not present\n");
		goto out;
	}
out:
	return ret;
...
static int dump_fwerr_log(char *buf, int size)
{
	...
	if ((((err_status.data & 0xFFFF) == SWDTERR_IND) ||
		...
		output_str(ret, buf, size, out, "HW WDT expired");
		...
		output_str(ret, buf, size, out, "HW WDT debug data:\n");
		output_str(ret, buf, size, out, "===================\n");
		for (count = 0;
			count < MAX_NUM_LOGDWORDS + MAX_NUM_LOGDWORDS_EXTENDED;
			count++) {
			output_str(ret, buf, size, out, "DW%d:0x%08x\n",
	...
	output_str(ret, buf, size, out, "Fabric Error debug data:\n");
	output_str(ret, buf, size, out, "===================\n");
	...
	for (count = 0; count < num_flag_status; count++) {
		...
		if (prev_id != id || id == FAB_ID_UNKNOWN) {
			output_str(ret, buf, size, out, "%s", ptr);
			prev_id = id;
		}
				   ...
				   int *read)
{
	...
	*read = 0;
	...
	if (!log_offset)
		output_str(ret, buf, size, leave, "SCU Extra trace\n");

	start = MAX_NUM_ALL_LOGDWORDS + log_offset / sizeof(u32);
	...
	for (i = start; i < end; i++) {
		 ...
		 */
		output_str(ret, buf, size, leave, "EW%d:0x%08x\n", i,
			   *(log_buffer + i));
		*read += sizeof(u32);
					...
					int count, int *peof, void *data)
{
	...
	if (!offset) {
		...
		ret += dump_scu_extented_trace(buffer, count, trace_offset,
					       &read);
		trace_offset += read;
		if (!read || trace_offset >= sram_buf_sz)
			*peof = 1;
	}
	*start = buffer - 1;
