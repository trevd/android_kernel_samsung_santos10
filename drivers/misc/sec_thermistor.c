				   ...
				   char *buf)
{
	...
	if (info->pdata->get_adc_data) {
		adc = info->pdata->get_adc_data();
...
static int convert_adc_to_temper(struct sec_therm_info *info, unsigned int adc)
{
	...
	const struct sec_therm_adc_table *temp_adc_table;
	...
	temp_adc_table = info->pdata->adc_table;
	temp_adc_table_size = info->pdata->adc_arr_size;

	if (temp_adc_table[0].adc >= adc) {
		temp = temp_adc_table[0].temperature;
		goto temp_result;
	} else if (temp_adc_table[temp_adc_table_size-1].adc <= adc) {
	...
	while (low <= high) {
		mid = (low + high) / 2;
		if (temp_adc_table[mid].adc > adc)
			high = mid - 1;
		else if (temp_adc_table[mid].adc < adc)
			low = mid + 1;
		else {
