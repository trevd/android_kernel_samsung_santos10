...
int sipc4_mif_dump_dpram(struct io_device *);
int sipc4_mif_dump_log(struct modem_shared *, struct io_device *);

#define mif_irq_log(msd, map, data, len) \
	_sipc4_mif_irq_log(MIF_IRQ, msd, map, data, len)
#define mif_com_log(msd, format, ...) \
	_sipc4_mif_com_log(MIF_COM, msd, pr_fmt(format), ##__VA_ARGS__)
#define mif_time_log(msd, epoch, data, len) \
	_sipc4_mif_time_log(MIF_TIME, msd, epoch, data, len)

void sipc4_mif_ipc_log(enum mif_log_id,
	struct modem_shared *, const char *, size_t);
void _sipc4_mif_irq_log(enum mif_log_id,
	struct modem_shared *, struct mif_irq_map, const char *, size_t);
void _sipc4_mif_com_log(enum mif_log_id,
	struct modem_shared *, const char *, ...);
void _sipc4_mif_time_log(enum mif_log_id,
...
int sipc4_pr_buffer(const char *tag, const char *data, size_t data_len,
							size_t max_len);

/* print a sk_buff as hex string */
#define pr_skb(tag, skb) \
	sipc4_pr_buffer(tag, (char *)((skb)->data), (size_t)((skb)->len), \
			(size_t)16)

/* print a urb as hex string */
#define pr_urb(tag, urb) \
	sipc4_pr_buffer(tag, (char *)((urb)->transfer_buffer), \
			(size_t)((urb)->actual_length), (size_t)16)

/* flow control CMD from CP, it use in serial devices */
int sipc4_link_rx_flowctl_cmd(struct link_device *ld, const char *data,
			size_t len);


/* get iod from tree functions */

struct io_device *sipc4_get_iod_with_format(struct modem_shared *msd,
					enum dev_format format);
struct io_device *sipc4_get_iod_with_channel(struct modem_shared *msd,
					unsigned channel);

static inline struct io_device *link_get_iod_with_format(
			struct link_device *ld, enum dev_format format)
{
	struct io_device *iod = sipc4_get_iod_with_format(ld->msd, format);
...
{
	struct io_device *iod = sipc4_get_iod_with_channel(ld->msd, channel);
	return (iod && IS_CONNECTED(iod, ld)) ? iod : NULL;
}

/* insert iod to tree functions */
struct io_device *sipc4_insert_iod_with_format(struct modem_shared *msd,
			enum dev_format format, struct io_device *iod);
struct io_device *sipc4_insert_iod_with_channel(struct modem_shared *msd,
			unsigned channel, struct io_device *iod);

/* iodev for each */
typedef void (*action_fn)(struct io_device *iod, void *args);
void sipc4_iodevs_for_each(struct modem_shared *msd, action_fn action,
			void *args);

/* netif wake/stop queue of iod */
void sipc4_iodev_netif_wake(struct io_device *iod, void *args);
void sipc4_iodev_netif_stop(struct io_device *iod, void *args);

/* change tx_link of raw devices */
void sipc4_rawdevs_set_tx_link(struct modem_shared *msd,
			enum modem_link link_type);

__be32 sipc4_ipv4str_to_be32(const char *ipv4str, size_t count);

void sipc4_mif_add_timer(struct timer_list *timer, unsigned long expire,
		void (*function)(unsigned long), unsigned long data);

/* debug helper functions for sipc4, sipc5 */
void sipc4_mif_print_data(char *buf, int len);
...
void sipc4_print_ip4_packet(u8 *ip_pkt);
bool sipc4_is_dns_packet(u8 *ip_pkt);
bool sipc4_is_syn_packet(u8 *ip_pkt);
