/*
 * platform_ctp_audio_wm1811.c: CLVS audio platform data initilization file
 *
...
void *ctp_audio_wm1811_platform_data(void *info)
{
	...
	ret = platform_device_add(pdev);
	if (ret) {
		pr_err("failed to add ctp_audio platform device\n");
		platform_device_put(pdev);
		return NULL;
	}
