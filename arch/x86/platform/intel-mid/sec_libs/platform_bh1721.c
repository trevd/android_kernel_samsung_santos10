...
static int bh1721fvc_light_sensor_reset(void)
{
	...
	pr_info("bh1721_light_sensor_reset !!\n");
...
static struct bh1721fvc_platform_data bh1721_pdata = {
	.reset = bh1721fvc_light_sensor_reset,
};

void __init *bh1721fvc_platform_data(void *info)
{
	struct i2c_board_info *i2c_info = info;

	gpio_als_nrst = get_gpio_by_name("ALS_NRST");
	pr_debug("%s addr : %x  gpio : %d\n", __func__, i2c_info->addr, gpio_als_nrst);
	if (gpio_als_nrst == -1)
		return NULL;
