...
#	+TARGET_TOOLS_PREFIX="$(ANDROID_BUILD_TOP)/$(TARGET_TOOLS_PREFIX)" TARGET_DEVICE="$(TARGET_DEVICE)" TARGET_BOARD_PLATFORM="$(TARGET_BOARD_PLATFORM)" KERNEL_SRC_DIR="$(KERNEL_SRC_DIR)" DIFFCONFIGS="$(DIFFCONFIGS)" KERNEL_DIFFCONFIG_DIR="$(KERNEL_DIFFCONFIG_DIR)" PRODUCT_OUT=`pwd`/$(PRODUCT_OUT) vendor/intel/support/kernel-build.sh $(KERNEL_BUILD_FLAGS) -o $@
